...
# Optional viewer (available in mujoco >= 2.3)
try:
...
def set_free_body_xyzyaw(model: mj.MjModel, data: mj.MjData, body_name: str, q_xyzyaw: np.ndarray):
    """Map q=[x,y,z,yaw] to the free joint of body_name in data.qpos."""
    ...
    qadr = model.jnt_qposadr[free_j]
    x, y, z, yaw = float(q_xyzyaw[0]), float(q_xyzyaw[1]), float(q_xyzyaw[2]), float(q_xyzyaw[3])
    half = 0.5 * yaw
    qw, qx, qy, qz = np.cos(half), 0.0, 0.0, np.sin(half)
    data.qpos[qadr:qadr+7] = np.array([x, y, z, qw, qx, qy, qz])


def preview_once(xml_path: str,
                 body_to_move: str,
                 q0: np.ndarray,
                 qT: np.ndarray,
                 make_vpsto_callable,
                 loss_fn_factory,
                 preview_max_iter: int,
                 preview_fps: int):
    """
    Solve once (cold) with a lightweight VPSTO, then animate the resulting trajectory in the MuJoCo viewer.
    Close the window to return.
    """
    if not HAS_VIEWER:
        print("MuJoCo viewer not available. Install mujoco>=2.3 and try again.")
        return

    # Load model fresh for the viewer (isolated from benchmark)
    m = mj.MjModel.from_xml_path(xml_path)
    d = mj.MjData(m)

    # Make a lightweight VPSTO for preview
    vpsto = make_vpsto_callable()
    # Temporarily reduce max_iter for quick preview (if user didn't already override)
    if hasattr(vpsto, "opt") and preview_max_iter is not None:
        vpsto.opt.max_iter = preview_max_iter

    loss_fn = loss_fn_factory(vpsto)
    print("[Preview] Solving once for visualization...")
    sol = run_one_solve(vpsto, loss_fn, q0, qT)
    q_via_best = sol.p_best

    # Generate dense trajectory (T, ndof)
    q_traj, _, _ = vpsto.vptraj.get_trajectory(q_via_best, q0, qT=qT, dqT=np.zeros_like(q0))
    q_traj = np.squeeze(q_traj, axis=0)

    dt = 1.0 / max(1, preview_fps)
    print(f"[Preview] Trajectory points: {len(q_traj)}, anim FPS: {preview_fps}")

    with mjv.launch_passive(m, d) as viewer:
        t_prev = time.perf_counter()
        idx = 0
        while viewer.is_running():
            # advance at ~preview_fps
            t_now = time.perf_counter()
            if (t_now - t_prev) >= dt:
                t_prev = t_now

                set_free_body_xyzyaw(m, d, body_to_move, q_traj[idx])
                mj.mj_forward(m, d)

                viewer.sync()
                idx = (idx + 1) % q_traj.shape[0]


def main():
    ap = argparse.ArgumentParser(description="VP-STO benchmark (cold vs warm) with MuJoCo + user hooks. Optional preview viewer.")
    ...
    ap.add_argument("--body", required=True, help="Body name with FREE joint to move (e.g., 'gripper_collision_with_block/')")
    # VPSTO options
    ...
    # Preview
    ap.add_argument("--preview", action="store_true", help="Open MuJoCo viewer and animate one solution before benchmarking")
    ap.add_argument("--preview_iter", type=int, default=40, help="Max iterations for the preview solve")
    ap.add_argument("--preview_fps", type=int, default=60, help="Animation FPS in viewer")
    args = ap.parse_args()

    # Load hooks
    hooks_mod = load_hooks_module(args.hooks)

    # Load MuJoCo model for benchmarking
    m = mj.MjModel.from_xml_path(args.xml)
    _ = mj.MjData(m)  # hooks may create their own MjData per call
    ...
    # ---- Preview (optional) ----
    if args.preview:
        preview_once(args.xml, args.body, q0, qT,
                     make_vpsto_callable=make_vpsto,
                     loss_fn_factory=loss_factory,
                     preview_max_iter=args.preview_iter,
